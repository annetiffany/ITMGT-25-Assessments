...
def shift_by_letter(letter, letter_shift):
    ...
    if letter == " ":
        ...
        shifted = alphabet.index (letter) + alphabet.index (letter_shift)
        if shifted < 26:
            return (alphabet [shifted])
        else:
            return (alphabet [shifted - 26])
...
def scytale_cipher(message, shift):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    shift_remainder = len (message) % shift
    if shift_remainder == 0:
        adjusted_message = message
    else:
        adjusted_message = message + "_" * (shift - shift_remainder)
    encoded_message = ""
    for i in range (0, len (adjusted_message)):
        letter_index = (i // shift) + (len (adjusted_message) // shift) * (i % shift)
        encoded_letter = adjusted_message [letter_index]
...
def scytale_decipher(message, shift):
    ...
    for i in range (0, len (message)):
        letter_index = round((i // new_shift) + (len (message) // new_shift) * (i % new_shift))
        encoded_letter = message [letter_index]
        encoded_message = encoded_message + encoded_letter
    return (encoded_message)
